...
namespace Mvc5Project.DAL
{
    public class BlogRepository : IBlogRepository, IDisposable
    {
        ...
        public int LikeDislikeCount(string typeAndlike, string id)
        {
            ...
            {
                case "postlike":
                    return _context.PostLikes.Where(p => p.Post.Id == id && p.Like == true).Count();
                case "postdislike":
                    return _context.PostLikes.Where(p => p.Post.Id == id && p.Dislike == true).Count();
                case "commentlike":
        ...
        public void UpdatePostLike(string postid, string username, string likeordislike)
        {
            ...
            var postLike = _context.PostLikes.Where(x => x.Username == username && x.Post.Id == postid).FirstOrDefault();
            if (postLike != null)
            {
            ...
            else
            {
                ...
                switch (likeordislike)
                {
                    ...
                    case "like":
                        PostLike postLike_ = new PostLike();
                        postLike_.Username = username;
                        postLike_.Like = true;
                        postLike_.Dislike = false;
                        Post p = _context.Posts.Where(x=>x.Id == postid).FirstOrDefault();
                        postLike_.Post= p;
                        //Post p = new Post{ Id = postid};
                        //postLike_.Post = p;
                        ...
                        //    Dislike = false
                        //};
                        _context.PostLikes.Add(postLike_);
                        break;
                    case "dislike":
                        postLike = new PostLike() {  Username = username, Like = false, Dislike = true };
        ...
        public void DeletePostandComponents(string postid)
        {
            ...
            var postLikes = _context.PostLikes.Where(p => p.Post.Id == postid).ToList();
        ...
        {
            Post p = _context.Posts.Where(x => x.Id == comment.PageId).FirstOrDefault();
            comment.Post = p;
